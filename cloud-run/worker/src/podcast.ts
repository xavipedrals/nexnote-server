...
interface Args {
    ...
    /// When true (e.g. iOS DEBUG), APNs for this job uses sandbox only for
    /// the completion push. Otherwise Cloud Run `APNS_USE_SANDBOX` applies.
...
export async function runPodcastJob(args: Args): Promise<void> {
    ...
    try {
        ...
        const { data: note, error: noteErr } = await supabase
            ...
            .eq("id", noteId)
            .single();
        if (noteErr || !note) throw new Error(`note_load_failed: ${noteErr?.message}`);

        const sourceText = note.ai_summary?.trim() || note.raw_transcript?.trim() || "";
        if (!sourceText) throw new Error("note_has_no_content");
        ...
        const { data: finalized, error: doneErr } = await supabase
            ...
            .single();
        if (doneErr) throw new Error(`finalize_failed: ${doneErr.message}`);

        console.log(`podcast ${podcastId} ready (${durationSecs}s)`);

        if (finalized) {
            const row = finalized as {
...
async function writeProgress(podcastId: string, pct: number): Promise<void> {
    ...
    if (clamped <= (data.progress_percent ?? 0)) return;
    await supabase
        .from("podcasts")
...
{"lines":[{"speaker":"host_a","text":"..."},{"speaker":"host_b","text":"..."}]}`;
    ...
    const parsed = JSON.parse(text) as { lines?: ScriptLine[] };
    const lines = (parsed.lines ?? []).filter(
        (l) => (l.speaker === "host_a" || l.speaker === "host_b") && l.text?.trim(),
    );
    return lines;
}
...
async function synthesizeScript(
    script: ScriptLine[],
    languageCode: string | null,
    onLineDone?: (lineIndex: number, totalLines: number) => void,
): Promise<Buffer> {
...
async function ttsLine(
    ...
    if (buf.length === 0) throw new Error("xai_tts_empty_audio");
    return buf;
}
...
// Helpers
// ---------------------------------------------------------------------------
function estimateDurationSecs(script: ScriptLine[]): number {
    const words = script.reduce(
